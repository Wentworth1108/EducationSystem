        ...
        $(function(){
            ...
            $('#t_course').datagrid({
                ...
                toolbar:[
					...
					handler:function(){
						flag = 'edit';
						var arr =$('#t_course').datagrid('getSelections');
						console.log(arr);
						if(arr.length != 1){
							$.messager.show({
								title:'提示信息!',
							...
							var userIds = [];
							for (var i = 0; i < arr[0].courseTeachers.length; i++) {
								userIds.push(arr[0].courseTeachers[0]['id']['user']['id']);
							}
							console.log(arr[0].courseTeachers[0]);
							$('#mydialog').dialog({
								title:'修改课程'
							});
							$('#mydialog').dialog('open'); //打开窗口
							$('#myform').get(0).reset();   //清空表单数据 
							$('#id').val(arr[0].courseId);
							$('#name').val(arr[0].name);
							...
							if(arr.length <=0){
								...
								$.messager.confirm('提示信息' , '确认删除?' , function(r){
										if(r){
											...
											console.log(ids);
											$.post('${pageContext.request.contextPath }/course/course_deleteSelected.action' , {ids : ids}, function(result){
												//1 刷新数据表格 
												$('#t_course').datagrid('reload');
												//2 清空idField   
												$('#t_course').datagrid('unselectAll');
												//3 给提示信息 
												$.messager.show({
													title:'提示信息!' , 
													msg:'操作成功!'
...
<div id="mydialog" title="新增课程" modal=true draggable=false class="easyui-dialog" closed=true  style="width:700px;">
    <form id="myform" action="" method="post">
    	<s:hidden id="id" name="course.courseId"/>
        <table align="center" style="margin-top: 15px;margin-bottom: 10px" data-options="toolbar:'#tb'">
            <tr>
                <td>课程名称:</td>
                <td><s:textfield id="name" name="course.name" class="easyui-validatebox" missingMessage="课程名必填!" required="true"/></td>
            </tr>
            <tr>
                <td>任课老师:</td>
                <td><s:checkboxlist list="#teacher" name="userIds" listKey="id.user.id" listValue="id.user.name"/></td>
            </tr>
